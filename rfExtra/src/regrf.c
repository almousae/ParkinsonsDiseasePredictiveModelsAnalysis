...
void regRF(double *x, double *y, int *xdim, int *sampsize,
	   ...
	   int *cat, int *maxcat, int *jprint, int *doProx, int *oobprox,
           int *biasCorr, double *yptr, double *errimp, double *impSD, 
           double *grperrimp, double *grpimpSD, double *impmat,
           double *prox, int *treeSize, int *nodestatus,
           int *lDaughter, int *rDaughter, double *avnode, int *mbest,
           double *upper, double *mse, int *keepf, int *replace,
           int *testdat, double *xts, int *nts, double *yts, int *labelts,
           double *yTestPred, double *proxts, double *msets, double *coef,
           int *nout, int *inbag, int *varGroups, int *nGroups) {
    /*************************************************************************
   Input:
  ...
  *************************************************************************/
    ...
    double *yb, *xtmp, *grpxtmp, *xb, *ytr, *ytree, *tgini;

    int k, m, mr, n, nOOB, j, jout, idx, ntest, last, ktmp, nPerm,
        nsample, mdim, keepF, keepInbag, g, grp;
    int *oobpair, varImp, grpvarImp, localImp, *varUsed;
    ...
    grpxtmp    = (double *) S_alloc(mdim * nsample, sizeof(double));
    ...
    for (j = 0; j < *nTree; ++j) {
		...
		if (grpvarImp) {
			for (g = 0; g < grp; ++g) {
				for (mr = 0; mr < mdim; ++mr){
					if (varGroups[mr] == (g+1) && varUsed[mr]){
						for (n = 0; n < nsample; ++n){
							grpxtmp[mr + n * mdim] = x[mr + n * mdim];
						}
					}
				}
				ooberrperm = 0.0;
				...
				delta = (ooberrperm / nPerm - ooberr) / nOOB;
                grperrimp[g] += delta;
                grpimpSD[g] += delta * delta;
                /* copy original data back */
                for (n = 0; n < nsample; ++n){
                	for (mr = 0; mr < mdim; ++mr){
						if (varGroups[mr] == (g+1) && varUsed[mr]){
							x[mr + n * mdim] = grpxtmp[mr + n * mdim];
						}
					}
                }             
			}
    ...
    if (grpvarImp) {
		for (g = 0; g < grp; ++g) {
			grperrimp[g] = grperrimp[g] / *nTree;
			grpimpSD[g] = sqrt( ((grpimpSD[g] / *nTree) -
							  (grperrimp[g] * grperrimp[g])) / *nTree );
        }
    }
}
