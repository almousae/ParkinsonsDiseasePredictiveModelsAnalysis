...
void classRF(double *x, int *dimx, int *cl, int *ncl, int *cat, int *maxcat,
             ...
             double *imprt, double *impsd, double *grpimprt, double *grpimpsd,
             double *impmat, int *nrnodes,
             ...
             int *inbag, int *varGroups, int *nGroups) {
  /******************************************************************
   ...
   *            cases will be split
   *  varGroups: integer vector of variable groups
   *
   *  Output:
   *
   ...
   *  grpimprt: matrix of group variable importance measures
  ...
  mimp, nimp, grpmimp, grpnimp, near, nuse, noutall, nrightall, nrightimpall,
  keepInbag, nstrata;
  int jb, j, n, m, k, idxByNnode, idxByNsample, imp, grpimp, localImp, iprox,
  oobprox, keepf, replace, stratify, trace, *nright,
  *nrightimp, *nout, *nclts, Ntree, g;

  int *out, *nodepop, *jin, *nodex,
  *nodexts, *nodestart, *ta, *ncase, *jerr, *varUsed,
  *jtr, *classFreq, *idmove, *jvr,
  *at, *a, *b, *mind, *nind, *jts, *oobpair;
  int **strata_idx, *strata_size, last, ktmp, nEmpty, ntry, grp;

  double av=0.0, delta=0.0;

  double *tgini, *tx, *grptx, *wl, *classpop, *tclasscat, *tclasspop, *win,
  *tp, *wr, *bestsplitnext, *bestsplit;
  ...
  mimp = imp ? mdim : 1;
  nimp = imp ? nsample : 1;
  grpmimp = grpimp ? grp: 1;
  grpnimp = grpimp ? nsample: 1;
  ...
  grptx =      (double *) S_alloc(mdim*nsample, sizeof(double));
  ...
  if (grpimp) {
    zeroDouble(grpimprt, (nclass+1) * grp);
    zeroDouble(grpimpsd, (nclass+1) * grp);
  }
  if (iprox) {
  ...
  for (jb = 0; jb < Ntree; jb++) {
    ...
    if (grpimp) {
      ...
      for (g = 0; g < grp; ++g) {
        ...
        zeroInt(nrightimp, nclass);      
        for (m = 0; m < mdim; ++m) {
          if (varGroups[m] == (g+1) && varUsed[m]) {
            for (n = 0; n < nsample; ++n) grptx[m + n*mdim] = x[m + n*mdim];
            /* Permute the m-th variable. */
            permuteOOB(m, x, jin, nsample, mdim);
          }
        }
        ...
        for (n = 0; n < nclass; ++n) {
          if (nout[n] > 0) {
            delta = ((double) (nright[n] - nrightimp[n])) / nout[n];
            grpimprt[g + n*grp] += delta;
            grpimpsd[g + n*grp] += delta * delta;
          }
        }
        /* overall measure, across all classes: */
        if (noutall > 0) {
          delta = ((double)(nrightall - nrightimpall)) / noutall;
          grpimprt[g + nclass*grp] += delta;
          grpimpsd[g + nclass*grp] += delta * delta;
        }
      }
    }
  ...
  /*  Final processing of group variable importance. */
  if (grpimp) {
    for (g = 0; g < grp; ++g) {
      /* class-specific measures */
      for (k = 0; k < nclass; ++k) {
        av = grpimprt[g + k*grp] / Ntree;
        grpimpsd[g + k*grp] =
          sqrt(((grpimpsd[g + k*grp] / Ntree) - av*av) / Ntree);
        grpimprt[g + k*grp] = av;
        /* grpimprt[g + k*grp] = (se <= 0.0) ? -1000.0 - av : av / se; */
      }
      /* overall measures */
      av = grpimprt[g + nclass*grp] / Ntree;
      grpimpsd[g + nclass*grp] =
        sqrt(((grpimpsd[g + nclass*grp] / Ntree) - av*av) / Ntree);
      grpimprt[g + nclass*grp] = av;
    }
  }
