...
randomForest <- function(x, y=NULL,  xtest=NULL, ytest=NULL, ntree=500,
             ...
             maxnodes=NULL,
             importance=FALSE, groupImp=FALSE, localImp=FALSE, nPerm=1,
    ...
    if (groupImp == TRUE && is.null(varGroups)){
        stop("No variable groups provided.")
    }
    grp <- 1
    if (groupImp == TRUE){
        grp <- nlevels(varGroups)
    }
    ...
    if (groupImp) {
        if (nPerm < 1) nPerm <- as.integer(1) else nPerm <- as.integer(nPerm)
        if (classRF) {
            grpimpout <- matrix(0.0, grp, nclass + 1)
            grpimpSD <- matrix(0.0, grp, nclass + 1)
        } else {
            grpimpout <- matrix(0.0, grp, 1)
            grpimpSD <- double(grp)
            names(grpimpSD) <- levels(varGroups)
        }
    } else {
        grpimpout <- double(grp)
        grpimpSD <- double(1)
    }
    ...
    if (classRF) {
        ...
        rfout <- .C("classRF",
                    ...
                    #DUP=FALSE,
                    varGroups = as.integer(varGroups),
                    nGroups = grp,
                    PACKAGE="rfExtra")[-1]
        if (keep.forest) {
        ...
        out <- list(call = cl,
                    ...
                    groupImportance = if(groupImp)
                    matrix(rfout$grpimpout, grp, nclass+1,
                           dimnames = list(levels(varGroups),
                           c(levels(y), "MeanDecreaseAccuracy")))
                    else NULL,
                    groupImportanceSD = if (groupImp)
                    matrix(rfout$grpimpSD, grp, nclass + 1,
                           dimnames = list(levels(varGroups),
                           c(levels(y), "MeanDecreaseAccuracy")))
                    else NULL,
        ...
        rfout <- .C("regRF",
                    ...
                    as.integer(ntree),
                    as.integer(mtry),
                    as.integer(c(importance, groupImp, localImp, nPerm)),
                    as.integer(ncat),
                    as.integer(maxcat),
                    as.integer(do.trace),
        ...
        out <- list(call = cl,
                    ...
                    groupImportance = if (groupImp) matrix(rfout$grpimpout, grp, 1,
                    dimnames=list(levels(varGroups),
                                  c("%IncMSE"))) else NULL,
                    groupImportanceSD=if (groupImp) rfout$grpimpSD else NULL,
                    localImportance = if (localImp)
